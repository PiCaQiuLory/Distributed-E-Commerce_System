...
public class TokenController {
	 ...
	 * @throws IOException
	 */
	@RequestMapping(value = "/user2/{token}"/*, produces = MediaType.APPLICATION_JSON_UTF8_VALUE*/)
	public void getUserByToken2(@PathVariable String token, String callback, HttpServletResponse response) {
		...
		try {
			...
			if (StringUtils.isNotBlank(callback)) {
				// 把结果封装成一个js语句响应
				String jsonString = callback + "(" + JsonUtils.objectToJson(e3Result) + ");";
				out.write(jsonString);
			} else {
				out.write(JsonUtils.objectToJson(e3Result));
			}
		} catch (IOException e) {
			e.printStackTrace();
		} finally {
